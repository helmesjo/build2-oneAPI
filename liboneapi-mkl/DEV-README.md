...
Also present in mkl-devel/lib/ but not packaged:
  libmkl_sycl.so  - GNU linker script pulling in all mkl_sycl_*.so.6 files;
                    not needed since mkl-sycl already exposes each lib directly.
...
## build2 targets
  ...
  mkl-gf-lp64              []                     (absent, GNU Fortran linux only)
  mkl-gf-ilp64             []                     (absent, GNU Fortran linux only)
  mkl-imalloc              (absent)               []
...
liboneapi-openmp{openmp} declared in buildfile (not package.json).
